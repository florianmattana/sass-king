// 14b_qmma_e4m3_e5m2_f32.cu
//
// Chapter 14 variant b. Same shape (m16n8k32) and same accumulator (FP32) as 14a,
// but mixed FP8 inputs: A is E4M3, B is E5M2. Tests whether SASS expresses
// per-operand input dtype distinctly.
//
// Matches CUTLASS SM120_16x8x32_TN<float_e4m3_t, float_e5m2_t, float>
// from mma_sm120.hpp (line 704).
//
// Fragment layout per thread (identical to 14a):
//   A: uint32_t[4] = 16 e4m3 per thread
//   B: uint32_t[2] = 8 e5m2 per thread (different element type, same register count)
//   C: float[4]
//   D: float[4]
//
...
//
// Compile: nvcc -arch=compute_120a -code=sm_120a -o 14b_qmma_e4m3_e5m2_f32 14b_qmma_e4m3_e5m2_f32.cu
// Dump:    cuobjdump --dump-sass 14b_qmma_e4m3_e5m2_f32 > 14b_qmma_e4m3_e5m2_f32.sass
...
__global__ void qmma_e4m3_e5m2_f32_kernel(
    ...
    float*          __restrict__ d)
{
    ...
    // The MMA under study. Only difference vs 14a: ".e5m2" replaces ".e4m3" for B.
    asm volatile(
...
int main(void) {
    ...
    // A: E4M3 1.0 = 0x38. Packed 0x38383838.
    ...
    qmma_e4m3_e5m2_f32_kernel<<<1, 32>>>(d_a, d_b, d_c, d_d);
