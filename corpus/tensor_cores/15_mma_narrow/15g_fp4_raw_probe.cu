// 15g_fp4_raw_probe.cu
//
// Raw FP4 E2M1 probe. Sweeps one nibble value repeated in A[0], holds B at
// 0x22222222, and prints the first D fragment. This gives direct evidence for
// which raw values produce nonzero output without assuming the full layout.
...
__global__ void qmma_e2m1_probe_kernel(
...
static uint32_t repeat_nibble(uint32_t x) {
    x &= 0xfu;
    uint32_t out = 0;
    for (int i = 0; i < 8; ++i) out |= x << (i * 4);
    return out;
}
