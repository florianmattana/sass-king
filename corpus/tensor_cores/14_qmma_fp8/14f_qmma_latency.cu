// 14f_qmma_latency.cu
//
// Chapter 14 variant f. QMMA serial latency microbenchmark. Analogue to 13e for HMMA.
// Chain of N QMMAs where each one's D feeds the next one's C, bracketed by clock64()
// to measure total cycles.
//
// Goal: measure cycles per serial QMMA on SM120, compare against HMMA's ~35 cycles.
//
// Compile with one of N=16, 32, 64 to fit the linear model:
//   nvcc -arch=compute_120a -code=sm_120a -DN_QMMA=16 -o 14f_qmma_latency_16 14f_qmma_latency.cu
//   nvcc -arch=compute_120a -code=sm_120a -DN_QMMA=32 -o 14f_qmma_latency_32 14f_qmma_latency.cu
//   nvcc -arch=compute_120a -code=sm_120a -DN_QMMA=64 -o 14f_qmma_latency_64 14f_qmma_latency.cu
//
// Dump:
//   cuobjdump --dump-sass 14f_qmma_latency_16 > 14f_qmma_latency_16.sass
//   etc.
...
// Helper macro: one QMMA in the chain (D = R0..R3 = C, A = a0..a3, B = b0..b1)
#define ONE_QMMA \
...
// Repeating ONE_QMMA N times via #define expansion
...
__global__ void qmma_latency_kernel(
...
int main(void) {
    ...
    // Each QMMA accumulates 32 products of 1.0 * 1.0 = +32 to the chain.
    // After N_QMMA chained: c = 0 + 32 * N_QMMA
    float expected = 32.0f * N_QMMA;
    printf("N=%d  total_cycles=%llu  cycles_per_qmma=%.2f  d[0]=%.1f (expected %.1f)\n",
           N_QMMA, h_cycles, (double)h_cycles / N_QMMA, h_d[0], expected);
